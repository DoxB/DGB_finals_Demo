...
export default function Chat() {
  ...
  useEffect(() => {
    // 첫 로딩 시 인사말 추가
    setMessages([{ sender: "bot", text: "안녕하세요! IM케어 AI보험설계사 우디입니다. 암보험 무엇이 궁금하신가요?" }]);
  }, []);
  ...
  const handleSend = async () => {
    ...
    const woudyEnd = process.env.NEXT_PUBLIC_WOUDY_END;
    const woudyPort = process.env.NEXT_PUBLIC_WOUDY_PORT;
    ...
    try {
      // POST 요청
      const response = await fetch(`${chatUrl}:${woudyPort}/${woudyEnd}`, {
        method: 'POST',
        headers: {
      ...
      const botResponse = data.answer; // 응답에서 'answer' 필드 추출
